...
def patchify(in_channels, imgs, patch_size, image_size):
    """
    imgs: (N, 4, D, H, W)
...
#     d = h = w = imgs.shape[2] // p #zlw
    d  = imgs.shape[2] // p #zlw
    h  = imgs.shape[3] // p
    w  = imgs.shape[4] // p
...
#     print('x.shape:',x.shape)
    ...
    imgs = x.reshape(shape=(x.shape[0], in_channels, d * p, h * p, w * p))
    return imgs
...
#     print('input:',x.shape,in_channels,patch_size,image_size)
    batch = x.shape[0]
    x_patch = patchify(in_channels, x, patch_size, image_size) #zlw加入参数image_size
...
#     print('1 mask_tokens.shape:',mask_tokens.shape)
    device = x.device
    mask_tokens = mask_tokens.repeat(batch,  id.shape[1] - mask_patch.shape[1], 1)
#     print('2 mask_tokens.shape:',mask_tokens.shape)
