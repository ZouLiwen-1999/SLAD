...
from unetr.network_architecture.neural_network import SegmentationNetwork
...
#             self.use_progress_bar = bool(int(os.environ['nnunet_use_progress_bar']))
    ...
    def plot_progress(self):
        ...
        try:
            ...
            #zlw保存epoch-tr_loss的值为csv文件以便于多个方法一起plot
            with open(join(self.output_folder, "progress.csv"),newline='',mode='w') as f:
                csv_writer = csv.writer(f, delimiter=',',quotechar='"',quoting=csv.QUOTE_MINIMAL)
                csv_writer.writerow(['Epoch','Tr_Loss']) 
                for x_value, zlw_tr_loss in zip(x_values, self.all_tr_losses):
                    csv_writer.writerow([x_value, zlw_tr_loss]) 
...
#         print('network_trainer.run_training()')
        _ = self.tr_gen.next()
        _ = self.val_gen.next()
        ...
        maybe_mkdir_p(self.output_folder)        
#         self.plot_network_architecture() #zlw不画网络架构了
        ...
        while self.epoch < self.max_num_epochs:
            self.print_to_log_file("Epoch {}/{}".format(self.epoch+1, self.max_num_epochs))
