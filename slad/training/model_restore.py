...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None):
    ...
    init = info['init']
    name = info['name']
    search_in = join(slad.__path__[0], "training", "network_training")
    tr = recursive_find_python_class([search_in], name, current_module="slad.training.network_training")
...
def load_model_and_checkpoint_files(folder, folds=None, mixed_precision=None, checkpoint_name="model_best"):
    ...
    print('##load model:',join(folds[0], "%s.model.pkl" % checkpoint_name))
